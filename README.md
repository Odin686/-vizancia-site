...
- `assets/privacy-consent.js` and `assets/privacy-consent.css` are the shared website consent implementation (v5, Google Consent Mode v2, strict opt-in). The script pushes a denied consent default first, treats a Global Privacy Control signal as a refusal that overrides any saved acceptance, removes the legacy `vizancia_google_ads_consent` key, and stores the visitor’s choice under `vizancia_consent_v2` for 180 days. Only after “Accept measurement” does it load gtag.js (GA4 `G-Z5P9FY92DE` and Google Ads `AW-18320211414`, personalised-advertising signals and Google Signals disabled) and record `app_store_click` / `play_store_click` events; “Essential only” or withdrawal pushes a denied consent update and clears the first-party Google cookies. Withdrawal also disables GA4 and reloads a tagged page to unload Google code; other tabs and restored pages check for withdrawal. Production tags run only on vizancia.com and www.vizancia.com. It must never be loaded with `async` or `defer`. `scripts/privacy.test.mjs` enforces these guarantees in CI through `npm run build`.
